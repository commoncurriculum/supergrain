...
- 6065b78: First release of `@supergrain/kernel`. The package was previously published as `@supergrain/core`; the React adapter (formerly `@supergrain/react`) is folded into the new `@supergrain/kernel/react` subpath, so `packages/react/` is gone. Same lineage, new name to match the rest of the rename to `kernel` / `silo` / `mill`.
  ...
  Also renames the `createStore` primitive to `createGrain` and reshapes the React integration around per-mount construction.

  **Breaking changes:**

  - `@supergrain/kernel`: `createStore(initial)` is renamed to `createGrain(initial)`. Same behavior, clearer vocabulary — the primitive builds a reactive proxy; the word "store" is reserved for the app-wide APIs in `@supergrain/kernel/react` and `@supergrain/silo`.
  - `@supergrain/kernel/react`: `provideStore(store)`, `StoreProvider`, the free-standing `useGranary`, and the `StoreRegistry` module-augmentation default singleton are all removed. Replace with `createGranaryContext<T>()`, which returns `{ Provider, useGranary }` tied to a fresh React Context. Pass your initial state to the Provider via the `initial` prop; the Provider wraps it in `createGrain(...)` exactly once per mount via the `useGrain` hook, so SSR requests, tests, and React trees are isolated by construction. Each factory call mints its own Context — sibling Providers don't collide, and there's no module-level singleton to leak across requests.

  **New:**

  - `@supergrain/kernel/react` ships `useGrain(initial)` for per-component reactive state. Wraps `createGrain` in `useState` so the proxy lives for the component's lifetime; no Provider needed for state scoped to a single component.
  ...
  **Migration — per-component state:**
  ...
  ```tsx
  import { useGrain } from "@supergrain/kernel/react";
