/**
 * Type-level tests for Branded<T> on createGrain return type.
 *
 * These tests verify that createGrain returns Branded<T> with $BRAND
...
import { createGrain, $BRAND, type Branded } from "../../src";
...
// ---------------------------------------------------------------------------
// 1. createGrain return type has $BRAND at top level
// ---------------------------------------------------------------------------

const simpleStore = createGrain({ name: "test", count: 0 });
...
const optionalStore = createGrain<{ title?: string | null; required: string }>({
  required: "yes",
});
...
// ---------------------------------------------------------------------------
// Runtime: existing behavior unchanged (createGrain still works at runtime)
// ---------------------------------------------------------------------------

describe("Branded type - runtime behavior unchanged", () => {
  it("createGrain still returns working reactive proxy", () => {
    const state = createGrain({ count: 0, nested: { value: "hello" } });
  ...
  it("arrays still work at runtime", () => {
    const state = createGrain({ items: [{ id: "1" }] });
