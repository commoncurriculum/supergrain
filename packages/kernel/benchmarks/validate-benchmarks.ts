...
async function validatePropertyUpdates() {
  ...
  // @supergrain/kernel
  const storableStore = createGrain({ count: 0 });
  let storableRuns = 0;
  ...
  console.log(
    `[@supergrain/kernel] Sequential updates: Expected 2 runs, got ${storableRuns}.`,
...
async function validateDeepUpdates() {
  ...
  // @supergrain/kernel
  const storableStore = createGrain(getDeepState());
  let storableRuns = 0;
  const storableDispose = effect(() => {
    storableRuns++;
  ...
  await nextTick();
  console.log(
    `[@supergrain/kernel] Deep updates: Expected 2 runs, got ${storableRuns}.`,
    storableRuns === 2 ? "PASS" : "FAIL",
  );
  storableDispose();
...
async function validateGranularReactivity() {
  ...
  // @supergrain/kernel
  const storableStore = createGrain(getInitialData());
  ...
  console.log(
    `[@supergrain/kernel] After update: Runs: ${storableRuns.join(", ")}.`,
