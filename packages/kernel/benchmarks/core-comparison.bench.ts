...
/**
 * Core benchmarks for comparing @supergrain/kernel with solid-js.
 *
 * IMPORTANT NOTES:
...
describe("Core: Property Access (Non-reactive)", () => {
  ...
  bench("@supergrain/kernel: 1M non-reactive reads", () => {
...
describe("Core: Reactive Effect Creation", () => {
  bench("@supergrain/kernel: create effect with 10k property reads", () => {
    ...
    if (runs !== 1) {
      validationError(`[@supergrain/kernel] Unexpected initial runs: ${runs}`);
    }
    dispose();
  });
...
describe("Core: Property Updates with Effects", () => {
  bench("@supergrain/kernel: 1000 sequential updates", async () => {
    ...
    if (runs !== 1) {
      validationError(`[@supergrain/kernel] Initial effect did not run. Runs: ${runs}`);
    }
    ...
    if (runs !== 2) {
      validationError(`[@supergrain/kernel] Expected 2 runs for batched updates, got ${runs}`);
    }
    dispose();
  });
...
describe("Core: Batch Updates", () => {
  bench("@supergrain/kernel: batch update 3 properties", async () => {
    ...
    if (runs !== 2) {
      validationError(`[@supergrain/kernel] Expected 2 runs, got ${runs}`);
    }
    dispose();
  });
...
describe("Core: Granular Reactivity", () => {
  bench("@supergrain/kernel: update one property in object with 10 properties", async () => {
    ...
    if (!passed) {
      validationError(
        `[@supergrain/kernel] Expected only one effect to run. Runs: ${runs.join(", ")}`,
      );
    }
  ...
  bench("solid-js/store: update one property in object with 10 properties", async () => {
    await testEffect((done) => {
      const data: any = {};
      for (let i = 0; i < 10; i++) data[`prop${i}`] = { nested: i };
