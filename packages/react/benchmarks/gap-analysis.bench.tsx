...
describe("Create 1000 rows - gap analysis", () => {
  ...
  bench("3. pure-dom + alien-signals", () => {
    ...
    for (const item of data) {
      ...
      cleanups.push(
        effect(() => {
          a1.textContent = labelSig();
        }),
      );
      cleanups.push(
        effect(() => {
          tr.className = selectedSig() ? "danger" : "";
        }),
      );
  ...
  bench("6. react-mount only", async () => {
    render(React.createElement("table", null, React.createElement("tbody", null)));
    cleanup();
  });
  ...
  bench("8. direct-dom (current)", async () => {
    ...
    const DirectDomApp: React.FC = () => {
      ...
      return React.createElement("table", null, React.createElement("tbody", { ref: tbodyRef }));
    };
