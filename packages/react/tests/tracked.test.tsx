...
describe('tracked()', () => {
  describe('per-component scoping', () => {
    it('label change re-renders only the affected Row, not App', async () => {
      ...
      const Row = tracked(({ item, label }: { item: RowData; label: string }) => {
        ...
        return <tr><td>{item.id}</td><td>{item.label}</td></tr>
      })
      ...
      const { container } = render(<table><tbody><App /></tbody></table>)
      await act(async () => {
        store.data = [{ id: 1, label: 'one' }, { id: 2, label: 'two' }]
      })
      ...
      await act(async () => { store.data[0].label = 'ONE UPDATED' })
    ...
    it('selection change re-renders App (reads selected) but only affected Rows via memo', async () => {
      ...
      const Row = tracked(({ item, isSelected }: { item: RowData; isSelected: boolean }) => {
        return <tr className={isSelected ? 'danger' : ''}><td>{item.id}</td><td>{item.label}</td></tr>
      })
      ...
      const { container } = render(<table><tbody><App /></tbody></table>)
      await act(async () => {
        store.data = [{ id: 1, label: 'one' }, { id: 2, label: 'two' }, { id: 3, label: 'three' }]
      })
  ...
  describe('structural operations', () => {
    ...
    function setup() {
      ...
      const result = render(<table><tbody><App /></tbody></table>)
      container = result.container.querySelector('tbody')!
    }

    it('create rows', async () => {
      setup()
      await act(async () => { store.data = [{ id: 1, label: 'a' }, { id: 2, label: 'b' }] })
    ...
    it('replace all rows', async () => {
      setup()
      await act(async () => { store.data = [{ id: 1, label: 'a' }] })
      await act(async () => { store.data = [{ id: 10, label: 'x' }] })
    ...
    it('clear all rows', async () => {
      ...
      await act(async () => { store.data = []; store.selected = null })
  ...
  describe('nested tracked components', () => {
    it('parent and child have independent subscriptions', async () => {
      ...
      await act(async () => { store.child = 'C UPDATED' })
      ...
      await act(async () => { store.parent = 'P UPDATED' })
