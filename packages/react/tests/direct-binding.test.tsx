...
describe("useDirectBindings", () => {
  it("updates textContent directly when signal changes", async () => {
    ...
    function TestComponent() {
      ...
      useDirectBindings([{ ref, getter: () => store.label }]);
      return (
        <span ref={ref} data-testid="label">
          {store.label}
        </span>
      );
    }
  ...
  it("supports multiple bindings on different elements", async () => {
    ...
    function TestComponent() {
      ...
      return (
        <div>
          <span ref={nameRef} data-testid="name">
            {store.name}
          </span>
          <span ref={ageRef} data-testid="age">
            {store.age}
          </span>
        </div>
      );
  ...
  it("works with computed expressions", async () => {
    ...
    function TestComponent() {
      const ref = useRef<HTMLSpanElement>(null);
      useDirectBindings([{ ref, getter: () => `${store.firstName} ${store.lastName}` }]);
