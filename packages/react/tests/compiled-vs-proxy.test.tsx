...
const CompiledApp: FC<{
  store: any;
  updateStore: any;
  removeFn: (id: number) => void;
  selectFn: (id: number) => void;
}> = memo(({ store, removeFn, selectFn }) => {
  appRenderCount++;
...
describe.each([
  ...
  it("create 1000 rows", async () => {
    ...
    render(
      <AppComponent store={store} updateStore={updateStore} removeFn={remove} selectFn={select} />,
    );
    resetTracking();
    await act(async () => {
      run(1000);
