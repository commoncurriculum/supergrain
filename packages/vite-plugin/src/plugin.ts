...
export function transformCode(
  ...
  for (const binding of ddBindings) {
    ...
    if (ts.isJsxSelfClosingElement(el)) {
      rewrites.push({
        ...
        replacement: ` ref={__$$${binding.index}}`,
      });
    } else {
      rewrites.push({
        start: insertPos,
  ...
  for (const [body, bindings] of bindingsByComponent) {
    ...
    const bindingEntries = bindings.map((b) => {
      const getterExpr =
        ts.isArrowFunction(b.callNode.arguments[0]) ||
        ts.isFunctionExpression(b.callNode.arguments[0])
          ? b.innerExprText
          : `() => ${b.innerExprText}`;
  ...
  if (hasDDBindings) {
    ...
    if (!code.includes("useRef")) {
      if (
        reactImportDecl?.importClause?.namedBindings &&
        ts.isNamedImports(reactImportDecl.importClause.namedBindings)
      ) {
    ...
    if (!code.includes("useDirectBindings")) {
      if (
        sgReactImportDecl?.importClause?.namedBindings &&
        ts.isNamedImports(sgReactImportDecl.importClause.namedBindings)
      ) {
