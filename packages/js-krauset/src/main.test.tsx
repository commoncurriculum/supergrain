...
describe('krauset compliance: react-supergrain', () => {
  ...
  beforeEach(() => {
    clear()
    const result = render(<table><tbody data-testid="tbody"><App /></tbody></table>)
  ...
  it('select row: select(id) sets class="danger"', async () => {
    ...
    await act(async () => { select(targetId) })
    const rows = getRows(container)
  ...
  it('remove row: remove(id) drops one row', async () => {
    ...
    await act(async () => { remove(targetId) })
    const rows = getRows(container)
