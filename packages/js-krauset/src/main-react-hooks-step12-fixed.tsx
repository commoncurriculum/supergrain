...
export const App = () => {
  ...
  // FIXED: Manage refs properly outside of useMemo to avoid hooks violations
  useEffect(() => {
    // Clear old refs when data structure changes
    const currentIds = new Set(state.data.map(item => item.id))
    for (const [id] of rowRefs.entries()) {
