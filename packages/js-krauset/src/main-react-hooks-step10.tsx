...
// OPTIMIZATION 9: Use startTransition for non-urgent updates
export const updateStore = updates => {
  if ('$set' in updates) {
    const setOps = updates.$set
    if ('data' in setOps && 'selected' in setOps) {
      // Large data operations are non-urgent - use startTransition
    ...
    // Handle update operations - these are non-urgent
    if (
    ...
    // Handle swap operations - these are non-urgent
...
export const App = () => {
  ...
  // Memoize the entire row list to avoid reconciliation work
  const rowElements = useMemo(() => {
    return state.data.map(item => (
      <Row key={item.id} item={item} selected={state.selected === item.id} />
    ))
