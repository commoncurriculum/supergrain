...
type LoosePathMap<K extends string, V> = Partial<Record<K, V>> &
  Record<string, V>
type LooseUnknownPathMap<K extends string, V> = Partial<Record<K, V>> &
...
export function splitPath(path: string): PathSegment[] {
  ...
  if (parts.some(part => part.length === 0)) {
    throw new Error(
      `Invalid update path "${path}". Empty path segments are not allowed.`
    )
  }
...
export function deleteValueAtPath(target: object, path: string): void {
  ...
  if (
    resolved &&
    Object.prototype.hasOwnProperty.call(resolved.parent, resolved.key)
  ) {
...
export type UnsetPathOperations<T extends object> = LoosePathMap<
  Path<T>,
  true | 1
>

export type NumericPathOperations<T extends object> = LoosePathMap<
  NumericPath<T>,
  number
>
...
export type ArrayWriteOperations<T extends object> = LooseUnknownPathMap<
  ArrayPath<T>,
  PathValue<T, ArrayPath<T>> extends Array<infer Item>
