...
function assertCompatibleTypedSchema(raw: object, entry: ModelProtoEntry): void {
  const cachedEntry = modelEntryCache.get(raw);
  if (cachedEntry && cachedEntry !== entry) {
    throw new Error("A raw object cannot be used with multiple typed store schemas.");
  }
}
...
function createModelViewFromEntry<T extends object>(raw: any, entry: ModelProtoEntry): T {
...
export function createModelView<T extends object>(raw: any, schema: SchemaLike): T {
