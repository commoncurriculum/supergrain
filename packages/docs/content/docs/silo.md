---
title: Silo
description: A reactive document cache for React — Suspense-compatible and request-batched.
---
...
- **Shipped feature count.** TQ has years of polish; we're shipping a read layer. If you need stale-time, refetch-on-focus, or mutations _today_, TQ wins. (Retry/backoff, cancellation, and pagination have since landed — pagination ships separately in [`@supergrain/queries`](/docs/queries).)
...
- Imperative `handle.refetch()` on a document — observe fresh data by calling `insertDocument` from a socket handler or a mutation response. (Paginated queries in [`@supergrain/queries`](/docs/queries) do expose `refetch()` / `fetchNextPage()`.)
- Server-push invalidation for individual documents — push fresh data with `insertDocument` instead. ([`@supergrain/queries`](/docs/queries) adds an opt-in `subscribe` hook for paginated queries.)
