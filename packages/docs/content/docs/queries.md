---
title: Queries
description: Pagination and live-subscription queries for Silo.
---

Pagination + live-subscription queries for [`@supergrain/silo`](/docs/silo). A reactive `createQuery` handle that pages through a resource, merges each page into the store, and refetches when the server says the results are stale — running on the **same Effect engine** as a silo document fetch.
...
For automatic lifecycle ownership (abort on rerun, dispose on unmount), the same pattern composes with [`@supergrain/husk`](/docs/husk)'s `useResource` / `defineResource`.
