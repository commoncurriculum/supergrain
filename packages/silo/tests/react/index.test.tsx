...
// =============================================================================
// Subscriber cancellation through the hooks
//
// useDocument/useQuery register a subscriber for the duration of the mount.
// When the component unmounts and no other subscriber wants the key, the
// in-flight fetch is interrupted — aborting the AbortSignal handed to the
// adapter. Proven end-to-end here (the engine-level behavior lives in
// tests/cancellation.test.ts); this file pins the hook wiring.
// =============================================================================

describe("hook-driven cancellation", () => {
  it("aborts the in-flight fetch when the only component using it unmounts", async () => {
    let signal: AbortSignal | undefined;
    const ctrlAdapter: DocumentAdapter = {
      find: (_ids, ctx) =>
        new Promise<Array<User>>((_resolve, reject) => {
          signal = ctx?.signal;
          ctx?.signal.addEventListener("abort", () => reject(new Error("aborted")));
        }),
    };
    const ctx = createDocumentStoreContext<DocumentStore<TypeToModel>>();
    const Badge = tracked(function Badge() {
      const handle = ctx.useDocument("user", "9");
      return <span>{handle.isFetching ? "loading" : "idle"}</span>;
    });

    const { unmount } = render(
      <StrictMode>
        <ctx.Provider config={{ models: { user: { adapter: ctrlAdapter } } }}>
          <Badge />
        </ctx.Provider>
      </StrictMode>,
    );
    ...
    await tick();
    expect(screen.getByText("loading")).toBeDefined();
    expect(signal?.aborted).toBe(false); // survives StrictMode's transient remount

    unmount();
    await tick();

    expect(signal?.aborted).toBe(true); // the fetch was torn down on unmount
  });

  it("keeps the fetch alive while another component still wants the same doc", async () => {
    let signal: AbortSignal | undefined;
    const ctrlAdapter: DocumentAdapter = {
      find: (_ids, ctx) =>
        new Promise<Array<User>>((_resolve, reject) => {
    ...
    const ctx = createDocumentStoreContext<DocumentStore<TypeToModel>>();
    const Badge = tracked(function Badge({ tag }: { tag: string }) {
      const handle = ctx.useDocument("user", "9");
      return <span>{`${tag}:${handle.isFetching ? "loading" : "idle"}`}</span>;
    });
    ...
    function Two({ showSecond }: { showSecond: boolean }) {
      return (
        <ctx.Provider config={{ models: { user: { adapter: ctrlAdapter } } }}>
          <Badge tag="a" />
          {showSecond ? <Badge tag="b" /> : null}
        </ctx.Provider>
      );
    }

    const { rerender } = render(<Two showSecond={true} />);
    await tick();
    expect(signal?.aborted).toBe(false);

    rerender(<Two showSecond={false} />); // one consumer leaves, one stays
    await tick();
    expect(signal?.aborted).toBe(false); // still wanted by <Badge a> → not cancelled
  });
});
