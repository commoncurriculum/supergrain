...
export interface AdapterRunOptions {
   ...
   * for telemetry / handle bookkeeping; it must not throw.
   */
 ...
 *   `retry` re-runs only *retryable* failures; `deadline` caps the whole loop.
 */
export function runAdapter<A>(
  invoke: (ctx: { signal: AbortSignal }) => Promise<A> | Effect.Effect<A, AdapterError>,
  options: AdapterRunOptions,
): Effect.Effect<A, AdapterError> {
  const { type, keys, retry, timeout, deadline, onFailure } = options;
  ...
  const observed =
    onFailure === undefined
      ? timed
      : timed.pipe(Effect.tapError((error) => Effect.sync(() => onFailure(error))));

  // Retry only retryable failures; a hard failure (e.g. a 4xx the adapter
  // marked `retryable: false`) fails fast instead of looping.
  const retried =
    retry === undefined
      ? observed
