...
Store-wide, `DocumentStoreConfig` also takes **`maxConcurrency`** (`number | "unbounded"`, default `"unbounded"`) — caps how many `adapter.find` chunks fan out at once when a large render produces many chunks — and **`onError`**, an error sink called on **every failed attempt** (not just on give-up) plus terminal `NotFoundError` / `ProcessorError`, with `{ type, keys, attempt, retryable }`. Use `attempt` / `retryable` to chart retry rate or alert only on hard (`retryable: false`) failures.
