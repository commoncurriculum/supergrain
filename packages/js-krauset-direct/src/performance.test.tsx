/**
 * Krauset performance tests — react-supergrain-direct (DirectFor).
 *
...
import {
  run, add, update, clear, swapRows, remove, select,
  App,
} from './main'

function getRowCount(tbody: HTMLElement) {
...
describe('krauset perf: react-supergrain-direct', () => {
  ...
  it('select row', async () => {
    mount()
    await act(async () => { run(1000) })
    const firstId = Number(tbody.querySelector('tr td')!.textContent)
  ...
  it('remove row', async () => {
    ...
    expect(getRowCount(tbody)).toBe(999)
  ...
  it('create many rows (10000)', async () => {
    ...
    expect(getRowCount(tbody)).toBe(10000)
  ...
  it('append rows (1000 to 1000)', async () => {
    ...
    expect(getRowCount(tbody)).toBe(2000)
  ...
  it('clear rows', async () => {
    ...
    expect(getRowCount(tbody)).toBe(0)
